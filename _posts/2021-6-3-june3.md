...
categories: [Learning, Statistics]
---
...
**.mean( )**  
When the dataset consists of the full population, the mean is represented by ***&mu;*** (*mu*), and the formula is:  

\begin{equation}\mu = \frac{\displaystyle\sum_{i=1}^{N}X_{i}}{N}\end{equation}  

More commonly, when working with a sample, the mean is represented by ***x&#772;*** (*x-bar*), and the formula is (note the lower case letters used to indicate values from a sample):  
...
When there is an **odd** number of observations, find the position of the median value using this formula (*n* is the number of observations):

\begin{equation}\frac{n+1}{2}\end{equation}

This formula returns the ***position*** of the median value in the sorted list; not the value itself.

If the number of observations is **even**, calculate the median as the average of the two middle-most values:
...
We can measure **skewness** (in which direction the data is skewed and to what degree) using **df[col].skew( )**   
and **kurtosis** (how "peaked" the data is) using **df[col].kurt( )**  .
```python
%matplotlib inline
import pandas as pd
import numpy as np
...
For a full population, it's indicated by *sigma* (***&sigma;<sup>2</sup>***) and calculated like this:  
...
It's calculated like this for a full population:
...
We call this a *Z Score*, and it's calculated like this for a full population:  
...
We can also use **.describe( )** to retrieve summary statistics for all numeric columns in a dataframe.  
These summary statistics include many of the statistics we've examined.  
```python
import pandas as pd
df = pd.DataFrame({'Name': ['Dan', 'Joann', 'Pedro', 'Rosie', 'Ethan', 'Vicky', 'Frederic'],
                   'Salary':[50000,54000,50000,189000,55000,40000,59000],
